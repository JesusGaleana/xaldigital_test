...
def validate_db_table(connection, df_data, query):
    ...
    Joins the DataFrame from the CSV file with the DataFrame from the database based on department names.
    ...
    if not df.empty:
        df = df[[ "department_name"]]
        merged_df = pd.merge(df_data, df, on=["department_name"], how="left", indicator=True)
...
if __name__ == "__main__":
    ...
    data_department = data[["department"]].drop_duplicates()
    ...
    df_new_data = validate_db_table(connection, data_department, query)
    ...
    if not df_new_data.empty:
        ...
        columns = "department_name"
        connection.insert_query(table="dim_department", columns=columns, values=values)
